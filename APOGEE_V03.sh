...
if [[ -f "${output_dir}/filteredPAFs/filtered_otu.tsv" ]]; then
    ...
    echo "  Parameters: -C ${confidence} -v ${qcov} -p ${identity_threshold}"
    ...
    mkdir -p "${output_dir}/filteredPAFs"
    time "${filter_script}" -i "${output_dir}/mapped" -o "${output_dir}/filteredPAFs/filtered_otu.tsv" -C "${confidence}" -v "${qcov}" -p "${identity_threshold}" || { echo "ERROR: PAF filtering with multi-filter failed" >&2; exit 1; }
...
if [[ -f "${output_dir}/phyloseq_taxonomy.csv" ]]; then
    ...
    "${taxonomy_script}" -i "${output_dir}/otu_table.csv" -t "${taxonomy_db}" -C "${confidence}" -v "${qcov}" -p "${identity_threshold}" > "${output_dir}/phyloseq_taxonomy.csv" 2>/dev/null || { echo "ERROR: Taxonomy table creation failed" >&2; exit 1; }
