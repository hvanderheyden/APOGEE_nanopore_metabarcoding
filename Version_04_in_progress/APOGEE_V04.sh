...
#chmod +x path/to/APOGEE_V04.sh

# /path/to/APOGEE_V04.sh \
# -i /path/to/fastq \
# -o /path/to/outputfolder \
# -r /path/to/ITS-RefDB_V03_fixed.mmi \
# -t 24 \
# -b false \
...
# -x 0.99 \
# -w 8 \
# -T /path/to/ITSRefDB_V03_taxonomy.tsv \
...
if [[ -f "${output_dir}/collapsed_otu_table.csv" ]]; then
    ...
    time "${collapse_script}" \
        -i "${output_dir}/filteredPAFs/filtered_otu.tsv" \
        -t "${output_dir}/filteredPAFs/phyloseq_taxonomy_filtered_otu.csv" \
    ...
    original_count=$(wc -l < "${output_dir}/filteredPAFs/filtered_otu.tsv")
