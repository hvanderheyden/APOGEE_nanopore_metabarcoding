...
def compute_lca_weighted(taxon_list, weight_list):
    ...
    for rank_idx, rank in enumerate(RANKS):
        ...
        # Accept if >40% weight agrees (lowered from 0.6 to handle multi-hit divergence in MOCK5)
        if vote_weight >= 0.4:
            lca_tax[rank] = value
            deepest_rank = rank_idx
        else:
...
for paf in paf_list:
    ...
    with open(paf, 'r') as f:
        for line in f:
            ...
            # Filter at hit level - REJECT LOW QUALITY ALIGNMENTS (MapQ < 10)
            if map_qual >= 10 and conf >= min_conf and qcov >= min_qcov and qlen_bp >= effective_min_qlen and identity >= min_identity:
                ...
                if log_filtering:
                    reason = []
                    if map_qual < 10:
                        reason.append(f"mapq={map_qual}<10")
                    if conf < min_conf:
